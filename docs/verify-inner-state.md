...
- [x] **LLM 通用拍照**（取代旧的「自拍集成 B」骨架）：pusher 无头入口 + Mirrai `generatePersonaPhoto`（带人/在家/外面三态）provider（`PERSONA_SELFIE_ENABLED`，默认关）。两条触发：LLM 在对话中自然输出 `[[PHOTO|...]]` 标记（受冷却/作息门控、深夜静默不主动）为主路径，规则正则（`发自拍/拍家里`，见 `selfie-decision.ts`/`selfie-commands.ts`）为必发兜底；区域库 `PERSONA_SELFIE_HOME_DIR` 按场景选卧室/厨房/客厅等参考图；发图走「文字预告 →（一两分钟）→ 实拍 → 交付语」三段式（`selfie-delivery.ts`）。
  - 验证：设 `PERSONA_SELFIE_ENABLED=true` 后，①明确说「发个自拍 / 拍个家里」应触发并走预告→图→交付；②聊到适合配图的情境，观察人物是否自然预告并异步发图；③深夜静默时段不应主动冒出拍照。
