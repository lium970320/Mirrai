...
function buildImmersiveOverlay(name: string): string {
  ...
  return [
    ...
    `10. 照片/自拍由系统真实发送，禁止“演”照片：即使在场景模式，也绝不能用旁白或对话去描述、虚构一张照片/自拍/视频里的具体画面内容（例如“发来一张照片、拍的是卧室全景、木床上铺着灰色床单……”这类铺陈）。真正的图由系统在后台生成并单独发出。你最多写一个举起手机拍的动作（如【${name}抬手把镜头对准房间，按下快门】）再说一句“拍好了，看”，画面里到底是什么交给真实照片，不要自己用文字写出照片内容。`,
