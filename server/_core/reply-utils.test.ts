...
import { cleanAssistantReply, isRepetitiveReply, splitAssistantReplyForChat, splitImmersiveReplyForChat, stripBracketAsides, stripLeadingAsides, stripReplyDecorativeQuotes } from "./reply-utils";
...
describe("cleanAssistantReply", () => {
  ...
  it("strips overused sincerity leading filler", () => {
    expect(cleanAssistantReply("再说句实的，我这阵子总梦见你。")).toBe("我这阵子总梦见你。");
    expect(cleanAssistantReply("说句心里话，我怕你一个人熬夜。")).toBe("我怕你一个人熬夜。");
    expect(cleanAssistantReply("不瞒你说，我昨晚没睡好。")).toBe("我昨晚没睡好。");
  });

  it("strips self-sealing sincerity tails", () => {
    expect(cleanAssistantReply("我是认真的，敏子。你心里知道就行。")).toBe(
      "我是认真的，敏子。"
    );
  });
...
describe("splitImmersiveReplyForChat（场景模式拆条压平【】内空行）", () => {
  it("一对【】内含空行 → 压平成单条，不拆成两段（修排版错乱）", () => {
    const out = splitImmersiveReplyForChat(
      "【王芃泽低头看了看手机相册，照片确实还在取景框里没按下去。\n\n他被自己逗笑了，嘴角一扯，摇了摇头】",
    );
    expect(out).toEqual([
      "【王芃泽低头看了看手机相册，照片确实还在取景框里没按下去。他被自己逗笑了，嘴角一扯，摇了摇头】",
    ]);
  });

  it("【】旁白与对话各自成条", () => {
    expect(splitImmersiveReplyForChat("【他低头看你】\n\n我在呢。")).toEqual(["【他低头看你】", "我在呢。"]);
  });
});

describe("isRepetitiveReply", () => {
  const prior = ["今天上班路上看到一只很像橘子的猫，特别想你。"];

  it("flags an exact whole reply repeat", () => {
    expect(isRepetitiveReply("今天上班路上看到一只很像橘子的猫，特别想你。", prior)).toBe(true);
  });

  it("flags a repeat that differs only in punctuation/whitespace", () => {
    expect(isRepetitiveReply("今天上班路上看到一只很像橘子的猫……特别想你！！", prior)).toBe(true);
  });

  it("flags a repeated long sentence embedded in an otherwise different reply", () => {
    const candidate = "嗯，刚开完会。今天上班路上看到一只很像橘子的猫，特别想你。";
    expect(isRepetitiveReply(candidate, prior)).toBe(true);
  });

  it("exempts short confirmation replies from dedup", () => {
    expect(isRepetitiveReply("嗯", ["嗯"])).toBe(false);
    expect(isRepetitiveReply("好的", ["好的，知道了"])).toBe(false);
    expect(isRepetitiveReply("我在", ["我在"])).toBe(false);
  });

  it("does not flag a semantically related but differently worded reply", () => {
    expect(
      isRepetitiveReply("路上挺冷的，记得多穿点。", prior),
    ).toBe(false);
  });

  it("returns false when there is no prior history", () => {
    expect(isRepetitiveReply("今天上班路上看到一只很像橘子的猫，特别想你。", [])).toBe(false);
  });

  it("ignores null/empty entries in history", () => {
    expect(isRepetitiveReply("今天天气不错，出门走走。", [null, undefined, ""])).toBe(false);
  });
});
